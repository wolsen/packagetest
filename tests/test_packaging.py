...
def _package_definition() -> PackageDefinition:
    return PackageDefinition(
        ...
        packaging_branch="debian/caracal",
    )
...
def test_build_package_operation_plan_uses_branch_and_debian_version(tmp_path: Path):
    ...
    assert plan.packaging_branch == "debian/caracal"
    ...
    assert plan.packaging_branch_file == tmp_path / "glance" / "packaging-branch.txt"
...
def test_build_package_operation_plan_uses_debian_orig_tarball_basename(tmp_path: Path):
    ...
    assert plan.packaging_branch is None


def test_build_package_operation_plan_allows_default_branch_discovery(tmp_path: Path):
    plan = build_package_operation_plan(
        package=PackageDefinition(
            source_package="glance",
            binary_packages=["glance"],
...
def test_package_operation_commands_cover_milestone_two_steps(tmp_path: Path):
    ...
    assert not any("ls-remote --symref" in text for text in command_texts)
    assert any("printf '%s\\n' debian/caracal" in text for text in command_texts)
    ...
    assert any("gbp import-orig --upstream-branch=upstream --pristine-tar --no-interactive --debian-branch=\"$branch\" --upstream-version=30.0.0.0b1" in text for text in command_texts)
...
def test_classify_packaging_failure_detects_patch_and_build_failures():
    ...
    assert classify_packaging_failure(["bash", "-lc", "git ls-remote --symref https://example.invalid/repo HEAD"], "", "fatal") == "PACKAGING_POLICY_FAILURE"
    assert classify_packaging_failure(["bash", "-lc", "git -C /tmp/pkg checkout \"$branch\""], "", "error: pathspec 'master' did not match any file(s) known to git") == "PACKAGING_POLICY_FAILURE"
